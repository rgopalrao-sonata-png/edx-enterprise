...
class SendPendingAdminRemindersCommandTests(TestCase):
    ...
    @mock.patch('enterprise.management.commands.send_pending_admin_reminders.send_enterprise_admin_invite_reminders')
    ...
    def test_command_logs_results(self, mock_task):
        ...
        out = StringIO()
        django_call_command(self.command, stdout=out)
    ...
    def test_command_handles_no_reminders_sent(self, mock_task):
        ...
        output = out.getvalue()
        assert 'Successfully sent 0 reminder(s)' in output
    ...
    def test_command_handles_task_exception(self, mock_task):
        ...
        with self.assertRaises(ValueError):
            django_call_command(self.command, stdout=out, stderr=err)

        error_output = err.getvalue()
        assert 'Missing Braze configuration' in error_output or 'Failed to send reminders' in error_output
    ...
    def test_command_runs_synchronously(self, mock_task):
        """
        Test that the command calls the task function directly (synchronously), not via .delay().
        """
        mock_task.return_value = {
            'processed': 0,
            'sent': 0,
        ...
        # Verify the function was called directly, not .delay()
        mock_task.assert_called_once_with()
        # Ensure it's not the Celery task's delay method
        assert not hasattr(mock_task, 'delay')
    ...
    @mock.patch('enterprise.management.commands.send_pending_admin_reminders.log')
    @mock.patch('enterprise.management.commands.send_pending_admin_reminders.send_enterprise_admin_invite_reminders')
    def test_command_logs_execution(self, mock_task, mock_log):
        ...
        # Check that starting message is logged
        assert any('Starting send_pending_admin_reminders' in msg for msg in log_messages)
        # Check that completion message is logged
        assert any('Completed send_pending_admin_reminders' in msg for msg in log_messages)
