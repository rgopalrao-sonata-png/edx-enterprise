...
# BrazeCampaignAPIClient is used for campaign-based emails (admin invites)
...
try:
    # Legacy Braze client errors for enterprise.api_client.braze.BrazeAPIClient code paths.
    from braze.exceptions import BrazeClientError as LegacyBrazeClientError
except ImportError:  # pragma: no cover - legacy braze package may not be installed in all envs.
    LegacyBrazeClientError = None

# Unified error used in except blocks for both campaign and legacy Braze clients.
if LegacyBrazeClientError is not None:
    BrazeClientError = (CampaignBrazeClientError, LegacyBrazeClientError)
else:
    # When legacy braze package is unavailable, legacy-client paths can fail with
    # AttributeError (e.g., missing methods on the fallback object client).
    BrazeClientError = (CampaignBrazeClientError, AttributeError)
